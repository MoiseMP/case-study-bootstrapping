...
for (n in n_values) {
  ...
  for (k in 1:length(break_sizes)) {
    delta <- break_sizes[k]
    
    # Break sizes are determined as constant times long run variance of independent variable s
    ...
    for (i in 1:n_loops) {
      ...
      file_name_txt_results <- here("output", "data", paste0("beta_plot_", file_name_txt))
      ...
      sink(file_name_txt_results, split = TRUE)
      ...
      cat('\nResults written to file:', file_name_txt_results, "\n")
    }
    ...
    # Save correct crossing point to file.
    filename_txt_crossing_point <- here("output", "data", paste0("crossing_point_plot", file_name_txt))
    
    sink(filename_txt_crossing_point, split = TRUE)
    
    for (i in 1:n_loops) {
      print("Crossing Point", i)
      print(failure_crossing_point(failure_rates_list[[i]], n/2, alpha))
    }
